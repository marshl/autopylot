...
def get_planet_plus_fleets(game_state: GameState, planet_id: int):
    return game_state.get_planet(planet_id).ships + \
           sum([fleet.ships for fleet in game_state.get_enemy_fleets() if fleet.destination_planet == planet_id]) - \
           sum([fleet.ships for fleet in game_state.get_my_fleets() if fleet.destination_planet == planet_id])
...
def get_commands(game_state: GameState):
    ...
    if len(game_state.get_my_planets()) / 2 > len(game_state.get_enemy_planets()):
        ...
        for idx, my_planet in enumerate(game_state.get_my_planets()):
            ...
            cmd = FleetCommand(my_planet.planet_id, enemy_planet.planet_id, my_planet.ships / 2)
            commands.append(cmd)
        return commands
    ...
    for planet in game_state.get_enemy_planets() + game_state.get_neutral_planets():
        ...
        score = 0
        if planet.ships > 0 and planet.player_id != game_state.current_player \
        ...
        if planet.player_id == game_state.current_player and get_planet_plus_fleets(game_state, planet.planet_id) < 0:
            dist = game_state.get_trip_length(source_planet.planet_id, planet.planet_id)
